...
class App extends Component {
  constructor(props) {
    super(props);
    this.state = { signalLeft: red, signalTop : red, signalRight: red,signalBottom: green, btn:"red", btn1:'red', btn2:'green', btn3:'red' };
  }
  
  componentWillMount(){
    var client  = mqtt.connect("ws://192.168.0.12:9001",{clientId:"mqttjs01"});
  ...
  changeSignalData(data){
    var client  = mqtt.connect("ws://192.168.0.12:9001",{clientId:"mqttjs01"});
    if(data==1){
      client.publish('fromCentralServer', '001100100100');
    }else if(data==2){
      client.publish('fromCentralServer', '100001100100');
    }else if(data==3){
      client.publish('fromCentralServer', '100100001100');
    }else if(data==4){
      client.publish('fromCentralServer', '100100100001');
    }else{
      client.publish('fromCentralServer', '000000000000'); 
    }
  }
    ...
    return (
      <div class="wrapper">
            ...
            <div className="signalController">
              ...
              <button className="signalControllerButton" style={{background: this.state.btn3}} onClick={()=>{this.changeSignalData(4)}}>ROAD 4</button>
            </div>
